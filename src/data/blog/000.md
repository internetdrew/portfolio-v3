...
dexie
  .version(1)
  .stores({ households: "id, title, created_at, creator_id, updated_at" });
...
export function useItems() {
  ...
  return { inventoryItems: user ? remoteItems : localItems || [], isLoading };
}
```
...
const STATUS_PRIORITIES = { OUT: 0, LOW: 1, HALFWAY: 2, FULL: 3 } as const;
  ...
  const endSession = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        await endSessionMutation.mutateAsync({ session_id: activeSession.id });
      } else {
...
![Screenshot of icon on mobile home screen](../../screenshots/post-0/screenshot-0-13-x.png)
![Screenshot native-like app UI](../../screenshots/post-0/screenshot-0-14-x.png)
